"""
Cumulative / batch-wise topic modeling for GraphWeave
====================================================

``CumulativeGraphWeave`` is a **separate** workflow that layers cumulative,
batch-wise clustering on top of the existing full-batch
:class:`~graphweave.GraphWeave` — without touching ``fit()`` /
...
are assigned with :meth:`GraphWeave.transform`.

The actual clustering engine is pluggable (see
:mod:`graphweave.cumulative.strategies`): ``global_refit`` (default), ``coreset``,
or ``batch_merge``. The "bigger picture" across accumulated data is delivered by
the existing :meth:`GraphWeave.build_hierarchy` and
:meth:`GraphWeave.generate_report_themes`, which scale with #topics, not #docs.
...
from graphweave.cumulative.strategies import (
    ReclusterContext,
    WorkingSet,
    make_strategy,
)
...
class CumulativeConfig:
    """Configuration for :class:`CumulativeGraphWeave`.
...
@dataclass
class BatchResult:
    """Returned by :meth:`CumulativeGraphWeave.add_batch`."""
...
@dataclass
class EpochSummary:
    """One row of :attr:`CumulativeGraphWeave.history_`."""
...
class CumulativeGraphWeave:
    ...
    config : CumulativeConfig, optional
        ...
        full-batch :class:`GraphWeaveConfig` reused for every recluster.

    Key attributes after one or more batches
    ----------------------------------------
    model_ : GraphWeave | None
    ...
    def __init__(self, config: CumulativeConfig | None = None):
        ...
        self.base_config = self.config.base_config or GraphWeaveConfig(verbose=self.config.verbose)
        ...
        # Fitted state.
        self.model_: GraphWeave | None = None
    ...
    def recluster(self) -> "CumulativeGraphWeave":
        ...
        new_model = GraphWeave(config=copy.deepcopy(self.base_config))
    ...
    def transform_proba(
        ...
        """Soft assignment over the current model's topics (delegates to GraphWeave)."""
        self._require_fitted()
    ...
    def bigger_picture(
        ...
        report meta-themes. Both reuse existing :class:`GraphWeave` machinery and
    ...
    def visualize(
        ...
        **kwargs,
        ...
        :meth:`GraphWeave.visualize` run on the same corpus.
    ...
    def visualize_topic_map(
        ...
        **kwargs,
        ...
        """
        from graphweave.visualization.plotter import plot_intertopic_distance_map
    ...
    def generate_labels(self, labeler, topics=None, dedup: bool = True, dedup_passes: int = 1):
        """Generate LLM labels for the current epoch's topics (delegates to GraphWeave).
    ...
    def generate_report_themes(self, labeler, n_themes=None, n_docs_per_theme: int = 12):
        """Synthesize high-level meta-themes from topic labels (delegates to GraphWeave).
    ...
    def regenerate_theme(self, labeler, theme_id: int, new_topic_ids=None, n_docs: int = 12):
        """Re-generate one meta-theme's narrative (delegates to GraphWeave).
    ...
    def reduce_outliers(self, strategy: str = "embeddings", threshold=None) -> "CumulativeGraphWeave":
        """Reassign outlier documents in the working set to the nearest topic.

        Mutates the internal GraphWeave model in-place. ``self.labels_`` and the
        global topic registry become stale until the next :meth:`recluster`.

        Parameters
        ----------
        strategy : str
            ``"embeddings"`` or ``"neighbors"`` — passed through to GraphWeave.
        threshold : float, optional
            Distance threshold passed through to GraphWeave.
        """
        self._require_fitted()
        self.model_.reduce_outliers(strategy=strategy, threshold=threshold)
        warnings.warn(
            "reduce_outliers() mutated the internal GraphWeave model. "
            "CumulativeGraphWeave.labels_ and the global topic registry are now stale. "
    ...
    def reduce_topics(self, n_topics: int) -> "CumulativeGraphWeave":
        """Merge working-set topics until ``n_topics`` remain.

        Mutates the internal GraphWeave model in-place. ``self.labels_``,
        ...
        warnings.warn(
            "reduce_topics() mutated the internal GraphWeave model. "
            "CumulativeGraphWeave.labels_ and the global topic registry are now stale. "
            "Call recluster() to re-synchronize.",
            UserWarning,
            stacklevel=2,
    ...
    def merge_topics(self, topics_to_merge: list[int]) -> "CumulativeGraphWeave":
        ...
        warnings.warn(
            "merge_topics() mutated the internal GraphWeave model. "
            "CumulativeGraphWeave.labels_ and the global topic registry are now stale. "
            "Call recluster() to re-synchronize.",
            UserWarning,
            stacklevel=2,
    ...
    def llm_merge_topics(
        ...
        Delegates to :meth:`GraphWeave.llm_merge_topics` on the internal model.
        Mutates the internal GraphWeave model in-place. ``self.labels_``,
        ``self._local_id_to_global``, and the global topic registry become stale
        until the next :meth:`recluster`.
        ...
        warnings.warn(
            "llm_merge_topics() mutated the internal GraphWeave model. "
            "CumulativeGraphWeave.labels_ and the global topic registry are now stale. "
            "Call recluster() to re-synchronize.",
            UserWarning,
            stacklevel=2,
    ...
    def _align_and_assign(self, new_model: GraphWeave, ws: WorkingSet) -> None:
    ...
    def __repr__(self) -> str:
        return (
            f"CumulativeGraphWeave(strategy={self.config.strategy!r}, epoch={self._epoch}, "
