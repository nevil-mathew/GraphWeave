# Cumulative / Batch-wise Clustering for GraphWeave
...
This is a **separate, additive** layer on top of GraphWeave. It **does not touch**
the existing full-batch `fit()` / `fit_transform()` — those behave exactly as
before. Everything here lives under `graphweave.cumulative`.
...
Use the original **full-batch** `GraphWeave.fit(documents)` when you already have the
...
flowchart TD
    ...
    Q -- "yes" --> RC["recluster():<br/>re-run the full GraphWeave pipeline on accumulated data,<br/>then ALIGN new topic IDs to the previous ones"]
...
The cumulative layer adds **no new dependencies** — it reuses GraphWeave's. For a
...
# or use the Google/API provider via GraphWeaveConfig(embedding_provider=...)
```
...
cfg = CumulativeConfig(
    base_config=GraphWeaveConfig(verbose=False),  # all the usual GraphWeave knobs
...
```python
model = CumulativeGraphWeave(CumulativeConfig())
model.add_batch(["doc one ...", "doc two ...", ...])   # embedded internally, once
```

> **No embedding model handy?** See `graphweave.cumulative.datasets.lsa_embed` and
...
flowchart TD
    ...
    C --> D["3 · new_model = GraphWeave(deepcopy(base_config))<br/>new_model.fit(ws.documents, ws.embeddings)<br/>identical full-batch pipeline"]
...
|---|---|---|
| `documents` / `embeddings` | the rows to fit the new `GraphWeave` on | step 3 (`fit`) |
...
**What the shared spine then does:** fits a fresh `GraphWeave`; because
...
`GraphWeave.fit(..., sample_weights=None)` (the default everywhere else) reproduces the old
...
After any number of batches, get the high-level view. This reuses GraphWeave's existing
...
# or:  pip install "graphweave[integration]"
```
...
### `CumulativeGraphWeave(config: CumulativeConfig | None = None)`
...
| `model_` | The most recent fitted `GraphWeave` (used for keywords/hierarchy/themes). |
...
`graphweave.cumulative.evaluation`: `compare_to_full_batch(...)`, `benchmark_strategies(...)`.
`graphweave.cumulative.datasets`: `make_streaming_corpus(...)`, `lsa_embed(...)`,
`StreamingCorpus`.
...
|---|---|---|
| `base_config` | `None` → `GraphWeaveConfig()` | The full-batch config reused for every recluster (graph type, consensus, HNSW backend, dim-reduction, embedder, …). |
...
`graphweave/core/model.py` is untouched; this is a separate package.
