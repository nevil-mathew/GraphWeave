...
strategies (see :mod:`graphweave.cumulative.strategies`):
...
None of this touches the full-batch :class:`~graphweave.GraphWeave` path.
"""
...
def _build_align_prompt(
    ...
    Mirrors the proposer style in :meth:`GraphWeave._propose_meta_themes`.
    """
    existing_lines = []
...
def _parse_alignment_response(
    ...
    :meth:`GraphWeave._parse_proposer_response`: ``json.loads`` on the ``{...}``
...
def assign_to_registry(
    ...
    Mirrors :meth:`GraphWeave.transform` but against the persistent global
...
def sensitivity_weights(embeddings: np.ndarray) -> np.ndarray:
    ...
    :meth:`~graphweave.core.clustering.ConsensusLeiden.fit_predict`), not just
...
def stratified_coreset(
    ...
    *labels* is the current global label per accumulated doc (``-1`` = outlier),
    ...
    That ``1 / p_i`` weight is passed through to ``GraphWeave.fit(sample_weights=...)``
    and from there into :meth:`~graphweave.core.clustering.ConsensusLeiden.fit_predict`,
