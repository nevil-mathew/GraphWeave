...
Neither class imports anything from ``graphweave.core`` — callers hand in a
duck-typed ``base_encoder`` (anything exposing ``.encode(list[str]) ->
np.ndarray``, e.g. :class:`graphweave.core.embeddings.EmbeddingEngine`) so this
...
class EmbeddingAdapter:
        ...
        if mode == "finetune":
            ...
            if dep_error is not None:
                raise ImportError(
                    ...
                    'Install with: pip install "graphweave[adaptation]", or use '
    ...
    def _finetune_sentence_transformer(self, documents: list[str], bank: TripletBank) -> None:
        ...
        output_dir = self.config.output_dir or tempfile.mkdtemp(prefix="graphweave_adapt_")
