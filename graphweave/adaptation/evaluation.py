...
def triplet_accuracy(embeddings: np.ndarray, judgments) -> float:
    ...
    Accepts a list of :class:`~graphweave.adaptation.triplets.TripletJudgment`
...
def _match_topics_by_centroid(centroids_a, centroids_b) -> list[tuple[int, int]]:
    ...
    detachable — mirrors ``graphweave.cumulative.evaluation._match_topics_by_centroid``.
    """
...
def _keyword_overlap_vs(model_a, model_b) -> float:
    from graphweave.utils.metrics import keyword_jaccard
...
def compare_embedders(
    ...
    """Fit an identical GraphWeave config on each variant's embeddings and
    ...
    from graphweave.utils.metrics import compute_ari, compute_nmi, compute_silhouette
    ...
    avg_rows = []
    first_seed_models: dict[str, "GraphWeave"] = {}
