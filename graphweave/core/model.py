...
from graphweave.utils.metrics import compute_coherence, compute_diversity, compute_stability
from graphweave.utils.timing import step_timer
from graphweave.utils.quote_verification import verify_quotes
...
class ReportTheme:
    ...
    Produced by :meth:`GraphWeave.generate_report_themes` for inclusion in a
    ...
    :func:`graphweave.utils.quote_verification.verify_quotes`."""


@dataclass
class GraphWeaveConfig:
    """Configuration for GraphWeave model."""
    ...
    # - "hnsw":   always hnswlib (requires `pip install graphweave[fast-knn]`).
...
class GraphWeave:
    """
    Multi-View Graph Topic Modeling with Iterative Refinement.
    ...
    Parameters
    ----------
    config : GraphWeaveConfig, optional
    ...
    >>> model = GraphWeave(n_neighbors=15, verbose=True)
    ...
    def __init__(
        self,
        config: GraphWeaveConfig | None = None,
        ...
        # Initialize config
        self.config = config or GraphWeaveConfig()
    ...
    def fit(
        ...
        if self.config.verbose:
            print(f"[GraphWeave] Fitting model on {n_docs} documents")
    ...
    def _compute_centroid_probabilities(self) -> None:
        ...
        from graphweave.utils.gpu import gpu_cosine_similarity
    ...
    def visualize_overlap(self, threshold: float = 0.1, **kwargs):
        ...
        """
        from graphweave.visualization.plotter import plot_topic_overlap
    ...
    def visualize_hierarchy_tree(self, **kwargs):
        ...
        fig : plotly.graph_objects.Figure
        """
        from graphweave.visualization.plotter import plot_hierarchy_tree
    ...
    def reduce_topics(self, n_topics: int) -> "GraphWeave":
        """
    ...
    def merge_topics(self, topics_to_merge: list[int]) -> "GraphWeave":
        """
    ...
    def tune_resolution_with_llm(
        ...
        from graphweave.labeling.llm_granularity import llm_select_resolution
    ...
    def adapt_embeddings_with_llm(
        ...
        :mod:`graphweave.adaptation` for the full machinery, including an
...
{{"narrative": "Your 4-6 sentence paragraph here."}}"""
    ...
    def visualize_topic_map(
        ...
        **kwargs
            Forwarded to :func:`graphweave.visualization.plotter.plot_intertopic_distance_map`
    ...
    @classmethod
    def load(cls, path: str) -> "GraphWeave":
    ...
    def __repr__(self) -> str:
        ...
        return f"GraphWeave(n_topics={n_topics}, status={status})"
